...
from codebase.acl_method.config import get_config
from codebase.acl_method.learner import Learner
import codebase.acl_method.models as md
...
#             print(x.size(), a.size(), y.size(), r.size())
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=1e-3, betas=(0.9, 0.999))
    if args.debias_mode == 'ACL':
        for i in model.parameters():
            i.requires_grad = False
        for i in model.acl_discriminito_t.parameters():
        ...
        for i in model.parameters():
            i.requires_grad = True
        for i in model.acl_discriminito_o.parameters():
            i.requires_grad = False
        for i in model.acl_discriminito_t.parameters():
            i.requires_grad = False
    ...
    for epoch in range(args.epoch_max):
        ...
        if args.debias_mode in ['Propensity_Mode']:
            ...
            for x, a, y, r, a_propensity in train_dataloader:
                ...
                # print(x_u.size(), count)
                loss = model.learn(x, a, r, y, x_u, a_u, y_u, a_propensity=a_propensity, savestep=epoch,
                                   save_flag=save_flag, train_turn='min')
                save_flag = False
                loss.backward()
                min_optimizer.step()
                ...
                # print(x_u.size(), count)
                loss = model.learn(x, a, r, y, x_u, a_u, y_u, a_propensity=a_propensity, savestep=epoch,
                                   save_flag=save_flag, train_turn='max')
                save_flag = False
                loss.backward()
                max_optimizer.step()
                m = len(train_dataloader)
        ...
        if epoch % 1 == 0:
            ...
            for test_x, test_a, test_y, test_r, negative_a in test_dataloader:
                ...
                if args.dataset == 'coat':
                    ...
                    test_ndcg = skm.ndcg_score(test_y.reshape(-1, 10), test_y_pred.reshape(-1, 10), k=10)
#                     print(test_ndcg)
